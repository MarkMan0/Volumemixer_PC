...
History: PJN / 23-02-1999 Code now uses QueryDosDevice if running on NT to determine
         ...
         PJN / 28-03-2016 1. Updated copyright details.
                          2. Updated CEnumerateSerial::RegQueryValueString to ensure that non null terminated data
returned from the registry API is null terminated before it is treated as such in the code.
         ...
         PJN / 28-05-2016 1. The sample app previously excluded support for CEnumerateSerial::UsingComDB for versions of
                          ...
                          Windows SDK which the code is being compiled against. This check is now performed by checking
the value of the VER_PRODUCTBUILD preprocessor value from the ntverp.h SDK header file. The sample app now excludes
support for CEnumerateSerial::UsingComDB on the Windows SDK 7.1 or earlier. This is because the msports.h header file is
only available with the Windows SDK 8 or later. Thanks to "scott" for reporting this issue. PJN / 02-07-2016 1. Updated
the SAL annotations in the code PJN / 25-09-2017 1. Updated copyright details.
         ...
         PJN / 15-01-2019 1. Updated copyright details.
                          ...
                          3. Fixed a compilation error "unknown identifier 'HDEVINFO'" in enumser.h. Thanks to Drew
Freer for reporting this issue. PJN / 22-06-2019 1. Updated the code to clean compile on VC 2019 PJN / 06-11-2019 1.
Updated initialization of various structs to use C++ 11 list initialization PJN / 17-03-2020 1. Updated copyright
details.
                          2. Fixed more Clang-Tidy static code analysis warnings in the code.
         PJN / 06-02-2021 1. Fixed a bug in CEnumerateSerial::QueryDeviceDescription where it would leave a trailing
null in the sFriendlyName output parameter. Thanks to "Thorsten" for reporting this bug.
                          2. Updated copyright details.
                          3. Updated the code to use std::vector::data method throughout. This means that the code must
now be compiled using /std:c++17. PJN / 05-03-2022 1. Updated copyright details.
...
#if !defined(NO_CENUMERATESERIAL_USING_SETUPAPI1) || !defined(NO_CENUMERATESERIAL_USING_SETUPAPI2) ||                  \
    !defined(NO_CENUMERATESERIAL_USING_REGISTRY)
...
_Return_type_success_(return != false) bool CEnumerateSerial::RegQueryValueString(_In_ ATL::CRegKey& key,
                                                                                  _In_ LPCTSTR lpValueName,
                                                                                  _Inout_ String& sValue) {
  // Reset the output parameter
  sValue.clear();
  ...
  // We will use RegQueryValueEx directly here because ATL::CRegKey::QueryStringValue does not handle non-null
  // terminated data
  DWORD dwType{ 0 };
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryRegistryPortName(_In_ ATL::CRegKey& deviceKey,
                                                                                    _Out_ int& nPort) {
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryUsingSetupAPI(_In_ const GUID& guid,
                                                                                 _In_ DWORD dwFlags,
...
_Return_type_success_(return != false) bool CEnumerateSerial::QueryDeviceDescription(_In_ HDEVINFO hDevInfoSet,
                                                                                     _In_ SP_DEVINFO_DATA& devInfo,
                                                                                     _Inout_ String& sFriendlyName) {
  DWORD dwType{ 0 };
  DWORD dwSize{ 0 };
...
#pragma warning(suppress : 26429)
_Return_type_success_(return != false) bool CEnumerateSerial::IsNumeric(_In_z_ LPCSTR pszString,
...
#pragma warning(suppress : 26429)
_Return_type_success_(return != false) bool CEnumerateSerial::IsNumeric(_In_z_ LPCWSTR pszString,
                                                                        _In_ bool bIgnoreColon) noexcept {
...
HRESULT CEnumerateSerial::UsingWMI(_Inout_ CPortAndNamesArray& ports) {
  ...
  hr =
      pLocator->ConnectServer(_bstr_t(R"(\\.\root\cimv2)"), nullptr, nullptr, nullptr, 0, nullptr, nullptr, &pServices);
  if (FAILED(hr)) return hr;
...
_Return_type_success_(return != false) bool CEnumerateSerial::UsingComDB(_Inout_ CPortsArray& ports) {
  ...
  #pragma warning(suppress : 26446)
  const LONG nStatus{ ComDBGetCurrentPortUsage(hComDB, portBytes.data(), dwMaxPortsReported, CDB_REPORT_BYTES,
                                               &dwMaxPortsReported) };
  if (nStatus != ERROR_SUCCESS) {
    ComDBClose(hComDB);
    SetLastError(nStatus);
...
_Return_type_success_(return != false) bool CEnumerateSerial::UsingRegistry(_Inout_ CNamesArray& ports) {
  ...
  #pragma warning(suppress : 26446)
    valueName[0] = _T('\0');
  #pragma warning(suppress : 26446)
    bContinueEnumeration = (RegEnumValue(serialCommKey, dwIndex, valueName.data(), &dwValueNameSize, nullptr, nullptr,
...
_Return_type_success_(return != false) bool CEnumerateSerial::UsingGetCommPorts(_Inout_ CPortsArray& ports) {
  ...
  if (pGetCommPorts == nullptr) return false;
