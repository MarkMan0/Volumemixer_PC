...
public:
  ...
  void launch(LPCTSTR serviceName) {
    ...
    mStatusHandle = RegisterServiceCtrlHandlerEx(serviceName, &Service::controlHandler, this);

    if (!mStatusHandle) return setStatus(SERVICE_STOPPED, ERROR_APP_INIT_FAILURE, 0);
    ...
    if (mApp->state()) return setStatus(SERVICE_STOPPED, ERROR_APP_INIT_FAILURE, 0);
...
private:
   ...
   */
  static DWORD WINAPI controlHandler(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext) {
    switch (dwControl) {
  ...
  void setStatus(DWORD dwCurrentState, DWORD dwWin32ExitCode, DWORD dwWaitHint) {
    ...
    if (dwCurrentState != SERVICE_START_PENDING) mStatus.dwControlsAccepted = SERVICE_ACCEPT_STOP;

    if (dwCurrentState != SERVICE_RUNNING || dwCurrentState != SERVICE_STOPPED) mStatus.dwCheckPoint = mCheckPoint++;
