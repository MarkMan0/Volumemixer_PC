...
namespace ServiceUtils {
  ...
  struct Setup {
     ...
     */
    int options(LPCTSTR serviceName, LPCTSTR displayName, LPTSTR description, TCHAR* argv[]) const {
  ...
  private:
    enum class Mode { Install, Uninstall, Create, Delete, Start, Stop };

    int options(LPCTSTR serviceName, LPCTSTR displayName, LPTSTR description, Mode mode) const {
    ...
    void create(LPCTSTR serviceName, LPCTSTR displayName, LPTSTR description, SC_HANDLE manager) const {
      ...
      auto service = CreateService(manager,                    // SCM database
      ...
      if (service) {
        ...
        std::cout << "CreateService error: " << win32Error(GetLastError()) << std::endl;
    }

    void tryDelete(LPCTSTR serviceName, SC_HANDLE manager) const {
      openService(serviceName, manager, DELETE, "DeleteService", "Service deleted.",
                  [](auto service) { return DeleteService(service) != 0; });
    }

    void start(LPCTSTR serviceName, SC_HANDLE manager) const {
      openService(serviceName, manager, SERVICE_START, "StartService", "Service running.",
                  [](auto service) { return StartService(service, 0, NULL) != 0; });
    }

    void stop(LPCTSTR serviceName, SC_HANDLE manager) const {
      openService(serviceName, manager, SERVICE_STOP, "ControlService", "Service stopped.", [](auto service) {
    ...
    void setDescription(LPCTSTR serviceName, SC_HANDLE manager, LPTSTR description) const {
      openService(serviceName, manager, SERVICE_CHANGE_CONFIG, "ChangeServiceConfig2", "Service description added.",
    ...
    void openService(LPCTSTR serviceName, SC_HANDLE manager, DWORD desiredAccess, const std::string& handlerName,
                     const std::string& successMessage, const std::function<bool(SC_HANDLE)>& handler) const {
      ...
      if (service) {
        ...
        std::cout << "Unable to run " << handlerName << " due to OpenService error: " << win32Error(GetLastError())
                  << std::endl;
    }
    ...
    void help(LPCTSTR serviceName) const {
      ...
      std::cout << "DESCRIPTION:" << std::endl
                << "        Command line program options for setting up this service with the Service Control Manager."
                << std::endl
                << "USAGE:" << std::endl
                << "        " << service << " <Option>" << std::endl
                << std::endl
                << "        Options:" << std::endl
                << "          install---------Creates this service as an Automatic startup service and sets it Running."
                << std::endl
                << "          uninstall-------Stops this service and deletes it." << std::endl
                << "          create----------Creates this service as an Automatic startup service." << std::endl
                << "          delete----------Deletes this service ie. completely removed from Service Control."
                << std::endl
