//
// ALL HAIL STACKOVERFLOW https://stackoverflow.com/questions/3438366/setupdigetdeviceproperty-usage-example
//
...
void ListDevices(CONST GUID* pClassGuid, LPCTSTR pszEnumerator) {
  ...
  for (i = 0;; i++) {
    ...
    if (SetupDiGetDeviceRegistryProperty(hDevInfo, &DeviceInfoData, SPDRP_HARDWAREID, &dwPropertyRegDataType,
                                         (BYTE*)szHardwareIDs,
    ...
    if (fn_SetupDiGetDevicePropertyW &&
      ...
      if (fn_SetupDiGetDevicePropertyW(hDevInfo, &DeviceInfoData, &DEVPKEY_Device_Manufacturer, &ulPropertyType,
                                       (BYTE*)szBuffer, sizeof(szBuffer), &dwSize, 0)) {
      ...
      if (fn_SetupDiGetDevicePropertyW(hDevInfo, &DeviceInfoData, &DEVPKEY_Device_LocationInfo, &ulPropertyType,
                                       (BYTE*)szBuffer, sizeof(szBuffer), &dwSize, 0)) {
        _tprintf(TEXT("    Device Location Info: \"%ls\"\n"), szBuffer);
      }
      if (fn_SetupDiGetDevicePropertyW(hDevInfo, &DeviceInfoData, &DEVPKEY_Device_SecuritySDS, &ulPropertyType,
                                       (BYTE*)szBuffer, sizeof(szBuffer), &dwSize, 0)) {
      ...
      if (fn_SetupDiGetDevicePropertyW(hDevInfo, &DeviceInfoData, &DEVPKEY_Device_ContainerId, &ulPropertyType,
                                       (BYTE*)szDesc, sizeof(szDesc), &dwSize, 0)) {
        StringFromGUID2((REFGUID)szDesc, szBuffer, ARRAY_SIZE(szBuffer));
        _tprintf(TEXT("    ContainerId: \"%ls\"\n"), szBuffer);
      }
      if (fn_SetupDiGetDevicePropertyW(hDevInfo, &DeviceInfoData, &DEVPKEY_DeviceDisplay_Category, &ulPropertyType,
                                       (BYTE*)szBuffer, sizeof(szBuffer), &dwSize, 0))
    ...
    while (pszToken != NULL) {
      ...
      if (szVid[0] != TEXT('\0')) _tprintf(TEXT("    vid: \"%s\"\n"), szVid);
      if (szPid[0] != TEXT('\0')) _tprintf(TEXT("    pid: \"%s\"\n"), szPid);
      if (szMi[0] != TEXT('\0')) _tprintf(TEXT("    mi: \"%s\"\n"), szMi);
...
int main() {
  ...
  _tprintf(TEXT("- Ports -\n"));
  _tprintf(TEXT("----------------------------\n"));
  ListDevices(&GUID_DEVCLASS_PORTS, NULL);
